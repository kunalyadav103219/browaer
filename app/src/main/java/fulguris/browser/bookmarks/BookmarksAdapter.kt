...
class BookmarksAdapter(
    ...
    override fun onBindViewHolder(holder: BookmarkViewHolder, position: Int) {
        // Check if the current position is within the visible range
        if (position < 2) {
            holder.itemView.visibility = View.GONE // Hide the itemView for the first two positions
        } else {
            holder.itemView.visibility = View.VISIBLE // Make the itemView visible for other positions
            // Adjust the position to skip the first two items
            val actualPosition = position + 2
            val viewModel = bookmarks[actualPosition]
    ...
    override fun getItemCount(): Int {
        val itemCount = bookmarks.size
        val skipCount = 2 // Number of items to skip
        return if (itemCount > skipCount) {
            itemCount - skipCount
        } else {
            0 // Return 0 if the number of items to skip is greater than or equal to the total count
        }
    }
    /**
