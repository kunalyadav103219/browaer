...
class MenuWebPage : PopupWindow {
    ...
    fun show(aAnchor: View) {
        ...
        // Retrieve anchor location
        val anchorLoc = IntArray(2)
        aAnchor.getLocationInWindow(anchorLoc)

        // Calculate yOffset and set gravity
        ...
        // Show the popup menu
        showAtLocation(aAnchor, gravity, fulguris.utils.Utils.dpToPx(10F), yOffset)

        // Animate the contentView from bottom to top
        val animator = ObjectAnimator.ofFloat(contentView, "translationY", yOffset.toFloat(), 0f)
        animator.duration = 500 // Set the animation duration (adjust as needed)
        animator.start()
