...
import fulguris.R
import fulguris.adapter.infintelayoutmananger
...
import fulguris.dbhelper.DatabaseHelper
import fulguris.di.*
...
abstract class WebBrowserActivity : ThemedBrowserActivity(),
    WebBrowser, OnClickListener, PreferenceFragmentCompat.OnPreferenceStartFragmentCallback,OnMenuItemClickListener {
    ...
    //
    @Inject
    open lateinit var tabsManager: TabsManager
   ...
   var bookmarkDatabase: BookmarkDatabase? =null
    ...
    private lateinit var nfintelayoutmananger: infintelayoutmananger
...
//        bookmarkDatabase!!.deleteAllBookmarks()
...
//       if(tabsManager.currentTab!!.url.equals("Harav://homee"))
//       {
...
//           iBinding.newui.visibility=View.GONE
//           iBinding.layoutHorizontal.visibility=View.VISIBLE
//       }


        //   val menuItem1 = iBinding.bottomNavigation.menu.findItem(R.id.nav_setting)

//        menuItem1.setOnMenuItemClickListener {
////            iBinding!!.appContainer.visibility= GONE
////            iBinding!!.appContainer2.visibility=GONE
////            iBinding!!.serchboxContainer.visibility=GONE
////            iBinding!!.toggle.visibility=GONE
////            iBinding!!.justBrowser.visibility= GONE
////            iBinding.pro.visibility= GONE
////            iBinding.tabsButton.visibility= GONE
////           val responsive=ResponsiveSettingsFragment()
////            supportFragmentManager
////                .beginTransaction()
////                .replace(R.id.container, responsive)
////                .runOnCommit {
...
//       recyclerView=findViewById(R.id.recyclerview)
        ...
        val configSettings = FirebaseRemoteConfigSettings.Builder()
            .setMinimumFetchIntervalInSeconds(100) // Adjust this interval according to your needs
            .build()
        ...
        remoteConfig.fetchAndActivate()
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    // Remote Config parameters fetched successfully
                    val adVersionFromRemoteConfig = remoteConfig.getString("ad_version")
                    val currentAdVersion = sharedPreferences.getString("ad_version", "")

                    // Check if ad version has changed in Remote Config
                    if (adVersionFromRemoteConfig != currentAdVersion) {
                        // Ad version has changed, update local ad version
                        sharedPreferences.edit().putString("ad_version", adVersionFromRemoteConfig).apply()
                        // Fetch data from Firestore and update SQLite
                        databaseHelper.clearAllData()
                        fetchDataFromFirestore()
//                        Toast.makeText(this,"Enter firesore1",Toast.LENGTH_LONG).show()
                    }
                    else if (databaseHelper.getAllData()==null)
                        {
                            fetchDataFromFirestore()
//                            Toast.makeText(this,"enter firestore2",Toast.LENGTH_LONG).show()
                        }
                    else
                    {
                        retrieveDataFromSQLite()
//                        Toast.makeText(this," enter retrives",Toast.LENGTH_LONG).show()
                    }

                        // Ad version has not changed, retrieve data from SQLit

                } else {
                    // Fetch failed, handle error
                    Log.e("YourActivity", "Fetch failed: ${task.exception}")
                    // Proceed to retrieve data from SQLite
...
// Carousel listener
      ...
      iBinding.carousel.carouselListener = object : CarouselListener {

            override fun onClick(position: Int, carouselItem: CarouselItem) {
                searchTheWeb(carouselItem.caption.toString())
            }
        ...
        iBinding.bottomNavigation1.setOnNavigationItemSelectedListener { menuItem ->
            val currentTab =
                tabsManager.currentTab ?: return@setOnNavigationItemSelectedListener false
            when (menuItem.itemId) {

                R.id.homee -> {
                    iBindingToolbarContent.addressBarInclude.search.setText("")
//                    currentTab.apply { requestFocus(); loadHomePage() }
//                   val webBrowser:WebBrowser?=null
//                    val Atab: WebPageTab? =null
//                    if (Atab != null) {
//                        onTabChanged(Atab).apply {
//                            webBrowser?.tabClicked(0)
//                        }
//                    }
...
//                    tabsManager.tabChanged(0, false, false)
...
//         val nagigatiohomebutton=findViewById<BottomNavigationView>(R.id.nav_home)
//        nagigatiohomebutton.isEnabled=false
        iBinding.bottomNavigation.setOnNavigationItemSelectedListener { menuItem ->
            // Handle navigation item selection here
            when (menuItem.itemId) {
                R.id.nav_home -> {
                    if (showingBookmarks()) {
                        tabsManager.getTabAtPosition(1)
                        closePanelBookmarks()
                ...
                R.id.nav_tabs -> {
                    if (configPrefs.tabBarInDrawer) openTabs() else {
                        showMenuWebPage()
                    }
                    false
                }
...
//        iBindingToolbarContent.tabsButton.setOnClickListener(this)
        ...
        searchView.setOnFocusChangeListener(searchListener)



//        iBindingToolbarContent.addressBarInclude.search.visibility=View.GONE
//        iBindingToolbarContent.addressBarInclude.searchSslStatus.visibility=View.GONE
//        iBindingToolbarContent.buttonMore.visibility=View.GONE
//        iBindingToolbarContent.buttonReload.visibility=View.GONE
//        iBindingToolbarContent.homeButton.visibility=View.GONE
//        iBindingToolbarContent.buttonActionBack.visibility=View.GONE
//        iBindingToolbarContent.tabsButton.visibility=View.GONE
//        iBindingToolbarContent.buttonActionForward.visibility=View.GONE
//        iBindingToolbarContent.contentt.visibility=View.GONE
    ...
    private fun retrieveDataFromSQLite() {

        if (databaseHelper.getAllData()==null)
        {
            fetchDataFromFirestore()
//            Toast.makeText(this,"retrives",Toast.LENGTH_LONG).show()
        }
        else
        {
//            Toast.makeText(this,"retriveSql",Toast.LENGTH_LONG).show()
            ...
            cursor?.let {
                if (it.moveToFirst()) {
                    do {
                        ...
                        val imageUrl = it.getString(it.getColumnIndex("imageUrl"))
                        val url = it.getString(it.getColumnIndex("searchUrl"))
                        val carouselItem = CarouselItem(imageUrl, url)

                        carouselItemList.add(carouselItem)
                        // Do something with the retrieved data, for example, log it
                        Log.d("YourActivity", "Image URL: $imageUrl, URL: $url")

                        // If you want to display the data in a list or perform other operations, you can do it here

                    } while (it.moveToNext()) // Move to the next row
                }
                iBinding.carousel.addData(carouselItemList)
    ...
    .document("carouselLinks")
    .get()
    .addOnSuccessListener { document ->
        if (document != null) {
            val links = document.get("links") as? List<Map<String, String>>
            links?.let {
                val linkItems = mutableListOf<LinkItem>()
                for (link in it) {
                    val imageUrl = link["imageUrl"]
                    val searchUrl = link["url"]
                    imageUrl?.let { imageUrl ->
                        searchUrl?.let { searchUrl ->
                            val linkItem = LinkItem(imageUrl, searchUrl)
                            val carouselItem = CarouselItem(imageUrl, searchUrl)
                            databaseHelper.insertData(searchUrl,imageUrl)
...
//        }
    ...
    private fun createMenuMain() {
        ...
        iMenuMain.apply {
            // Menu
            onMenuItemClicked(iBinding.menuItemWebPage)
            {
                dismiss(); showMenuWebPage()
            }
            // Bind our actions
            onMenuItemClicked(iBinding.menuItemSessions)
            {
                dismiss();
                executeAction(R.id.action_sessions)
            }
            ...
            //
            onMenuItemClicked(iBinding.menuItemSettings)
            {
                dismiss();
                executeAction(R.id.action_settings)
            }
            ...
            onMenuItemClicked(iBinding.menuShortcutForward) {
                iBinding.layoutMenuItemsContainer.isVisible =
                    false; executeAction(R.id.action_forward)
            }
            onMenuItemClicked(iBinding.menuShortcutBack) {
                iBinding.layoutMenuItemsContainer.isVisible = false; executeAction(R.id.action_back)
            }
...
//        searchView1.clearFocus()
    ...
    private fun showMenuWebPage() {
        ...
        currentTabView?.clearFocus()
//        searchView.clearFocus()
        iBinding.bottomNavigation1.clearFocus()


        // searchView1.clearFocus()
        // Show popup menu once our virtual keyboard is hidden
        doOnceVirtualKeyboardIsGone {
            doShowMenuWebPage()
        }
    }
    ...
    private fun doShowMenuWebPage() {
        iMenuWebPage.show(findViewById<View>(R.id.moree))
    }
...
//                    }
    ...
    private fun initialize(savedInstanceState: Bundle?) {
        ...
        val tabs = iBinding.bottomNavigation.menu.findItem(R.id.nav_tabs)
//        val tabs2 = iBinding.bottomNavigation1.menu.findItem(R.id.nav_tabs)
        tabs.setOnMenuItemClickListener(this)
//        tabs2.setOnMenuItemClickListener(this)

//        tabs.setOnMenuItemClickListener {
//            easyTabSwitcherStart()
//            false
//        }
//        tabs2.setOnMenuItemLongClickListener {
//            easyTabSwitcherStart()
//            false
//        }

        // iBinding.tabsButton.setOnClickListener(this)
        iBindingToolbarContent.tabsButton.setOnClickListener(this)
//        iBinding.justBrowser.setOnClickListener(this)
...
//        tabs2.setOnMenuItemClickListener {
//            if (iTabsButtonLongPressed) {
...
//        iBindingToolbarContent.buttonActionForward.setOnClickListener { executeAction(R.id.action_forward) }
        ...
        iTabViewContainerFront.setOnRefreshListener {
            ...
            mainHandler.postDelayed(
                { iTabViewContainerFront.isRefreshing = false },
                1000
            )   // Stop the loading spinner after one second
        }
...
//    {
//        override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
//            // Code here if you need to do something before the text changes
//        }
//
//        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
...
//
//            // Use this method to track changes in the SearchView's text in real-time.
//            // For example:
//            searchTheWeb(s.toString()) // Or any other operation you need to perform with the current text
//        }
//
//        override fun afterTextChanged(s: Editable?) {
//            // Code here if you need to do something after the text has changed
//        }
//
        ...
        override fun onFocusChange(v: View, hasFocus: Boolean) {
            ...
            if (currentView != null) {
                ...
                if (!hasFocus) {
                    ...
                    (v as SearchView).selectAll()
                    // iBindingToolbarContent.addressBarInclude.searchSslStatus.visibility = GONE
                    iBinding.searchSslStatuss.visibility = GONE
                }
            }

            if (!hasFocus) {
                iBindingToolbarContent.addressBarInclude.searchSslStatus.updateVisibilityForContent()
//                iBinding.searchSslStatuss.updateVisibilityForContent()
                searchView.let {
                    inputMethodManager.hideSoftInputFromWindow(it.windowToken, 0)
                }
//                searchView1.let {
...
//            }
    ...
    private fun showUrl() {
        ...
        if (tabsManager.currentTab!!.url.equals("Harav://homee")) {
            ...
            if (!url.isSpecialUrl()) {
//                searchView.setText(url)
                iBindingToolbarContent.addressBarInclude.search.setText(url)
                searchView.setText(url)
    ...
    private inner class DrawerLocker : DrawerLayout.DrawerListener {
        ...
        override fun onDrawerClosed(v: View) {
            ...
            if (v === tabsDrawer) {
                iBinding.drawerLayout.setDrawerLockMode(
...
//                searchView1.dropDownAnchor=R.id.address_bar_include
...
//                searchView1.dropDownAnchor=R.id.toolbar_include
    ...
    private fun setupToolBar() {
        ...
        (iBinding.toolbarInclude.toolbar.width > threshold).let { isVisible ->
//            iBindingToolbarContent.buttonActionBack.isVisible = isVisible
//            iBindingToolbarContent.buttonActionForward.isVisible = isVisible
            findViewById<View>(R.id.forward).visibility = if (isVisible) View.VISIBLE else View.GONE
            findViewById<View>(R.id.backward).visibility =
                if (isVisible) View.VISIBLE else View.GONE
            // Hide tab bar action buttons if no room for them
            if (tabsView is TabsDesktopView) {
                (tabsView as TabsDesktopView).iBinding.actionButtons.isVisible = isVisible
            }
        }
    }
...
//            if (searchView1.hasFocus()==true)
//            {
//                searchView1.let { searchTheWeb(it.text.toString()) }
//            }
    ...
    private fun tabSwitchApply(aGoingBack: Boolean) {
        iCapturedRecentTabsIndices?.let {
            if (iRecentTabIndex >= 0) {
                ...
                tabsManager.tabChanged(
                    tabsManager.indexOfTab(it.elementAt(iRecentTabIndex)),
                    false,
                    aGoingBack
                )
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        if (event.action == KeyEvent.ACTION_DOWN) {
            ...
            val isShiftOnly =
                KeyEvent.metaStateHasModifiers(event.metaState, KeyEvent.META_SHIFT_ON)
            val isCtrlShiftOnly = KeyEvent.metaStateHasModifiers(
                event.metaState,
                ...
                KeyEvent.KEYCODE_F3 -> {
                    if (isShiftOnly) {
                        ...
                        currentTabView?.evaluateJavascript("(function(){return window.getSelection().toString()})()",
                            ValueCallback<String> { aSelection ->
            ...
            when {
                isCtrlOnly -> when (event.keyCode) {
                    ...
                    KeyEvent.KEYCODE_D -> {
                        ...
                        tabsManager.currentTab?.let {
                            tabsManager.newTab(
                                FreezableBundleInitializer(TabModelFromBundle(it.saveState())),
                                true
                            )
                            tabSwitchReset()
                        }
    ...
    override fun executeAction(@IdRes id: Int): Boolean {
        ...
        when (id) {
            ...
            android.R.id.home -> {
                if (showingBookmarks()) {
//                   closePanelBookmarks()
//                    tabsManager.getTabAtPosition(0)
                    if (userPreferences.homepageInNewTab) {
                        if (isIncognito()) {
                            tabsManager.newTab(incognitoPageInitializer, true)
                        } else {
...
//                else if(searchView1.hasFocus())
//                {
//                    searchView1.hasFocus()
//                }
                else {
                    refreshOrStop()
                }
            ...
            R.id.action_block -> {
                ...
                abpUserRules.allowPage(
                    Uri.parse(tabsManager.currentTab?.url),
                    !iMenuWebPage.iBinding.menuItemAdBlock.isChecked
                )
                tabsManager.currentTab?.reload()
                return true
            }
    ...
    private fun updateReloadButton() {
        ...
        iBindingToolbarContent.buttonReload.isVisible =
            !iTabViewContainerFront.isEnabled || isLoading()
...
//                if (configPrefs.toolbarsBottom) Gravity.TOP else Gravity.BOTTOM
//            )
...
//                }.show()
    ...
    override fun updateSslState(sslState: SslState) {
        ...
        iBinding.searchSslStatuss.setImageDrawable(createSslDrawableForState(sslState))
...
//                alpha = if (isEnabled) 1.0f else 0.25f
//            }
            findViewById<View>(R.id.backward).apply {
            ...
            findViewById<View>(R.id.forward).apply {
...
//            }
     ...
     */
    override fun setTabView(
        aView: View,
        aWasTabAdded: Boolean,
        aPreviousTabClosed: Boolean,
        aGoingBack: Boolean
    ) {
        Timber.i("setTabView")
    ...
    override fun showSnackbar(@StringRes resource: Int) =
        snackbar(resource, if (configPrefs.toolbarsBottom) Gravity.TOP else Gravity.BOTTOM)

    fun showSnackbar(aMessage: String) =
        snackbar(aMessage, if (configPrefs.toolbarsBottom) Gravity.TOP else Gravity.BOTTOM)
...
//            iBindingToolbarContent.contentt.layoutParams = layoutParams
            ...
            iBinding.newui.visibility = View.GONE
            iBinding.layoutHorizontal.visibility = View.VISIBLE
//            val dpheight = 60 // Your desired width in dp
//            val layoutParams = iBinding.toolbarInclude.toolbarLayout.layoutParams
//            layoutParams.height = dpToPx(dpheight)
//            iBindingToolbarContent.contentt.layoutParams = layoutParams
    ...
    private fun getFirstFiveFromUrl(urlString: String): String? {
    ...
    private fun doBackAction() {
        ...
        else
        {
            ...
            if (iJustClosedMenu) {
                // Toast.makeText(this, "enter9", Toast.LENGTH_LONG).show()

                return
            }
            if (showingTabs()) {
                //Toast.makeText(this, "enter6", Toast.LENGTH_LONG).show()
                if (tabsManager.currentTab!!.url.equals("Harav://homee")) {
                    iBinding.layoutHorizontal.visibility = View.GONE
                    iBinding.newui.visibility = View.VISIBLE
                    iBindingToolbarContent.addressBarInclude.search.setText("")
...
//                             Toast.makeText(this, "enter s", Toast.LENGTH_LONG).show()
            } else {
                val previousTabPosition = tabsManager.positionOf(currentTab) - 1
                if (previousTabPosition >= 0) {
                    var previousTab =
                        tabsManager.getTabAtPosition(previousTabPosition)
                    val previousTabUrl = previousTab?.url
                    if (previousTabUrl != null) {
                        if (previousTabUrl == "Harav://homee") {
                            iBindingToolbarContent.addressBarInclude.search.setText(
                                ""
                            )
                            iBindingToolbarContent.tabsButton.isVisible = true
                            ...
                            iBindingToolbarContent.tabsButton.isVisible = true
                            iBinding!!.layoutHorizontal.visibility =
                                View.VISIBLE
                            iBinding!!.newui.visibility = View.GONE
...
//                        Toast.makeText(this, "enter s1 ${tabsManager.currentTab!!.url}", Toast.LENGTH_LONG).show()


                if (currentTab != null) {
                    Timber.d("onBackPressed")
                    if (searchView.hasFocus()) {
                        // Toast.makeText(this, "enter s22", Toast.LENGTH_LONG).show()
                        ...
                        if (!currentTab.isShown) {
                            ...
                            //  Toast.makeText(this, "enter2", Toast.LENGTH_LONG).show(


                        } else {
                            if (isToolBarVisible()) {
//                       Toast.makeText(this, "enter3", Toast.LENGTH_LONG).show()
                                currentTabGoBack()
//                            iBinding.newui.visibility= GONE
//                            iBinding.layoutHorizontal.visibility= VISIBLE
                                // After going back, check the URL to determine visibility.

                            } else {
//                                        Toast.makeText(this, "entersp3", Toast.LENGTH_LONG).show()
                                showActionBar()
                            }
                        }
                    } else {
...
//                               Toast.makeText(this, "p1", Toast.LENGTH_LONG).show()
                            ...
                            if (isToolBarVisible()) {
                                val previousTabPosition1 =
                                    tabsManager.positionOf(currentTab) - 1
                                var previousTab =
                                    tabsManager.getTabAtPosition(previousTabPosition1)
                                val previousTabUrl1 = previousTab?.url

                                if (previousTabPosition1 > 0) {
                                    if (previousTabUrl1 == "Harav://homee") {
                                        iBindingToolbarContent.tabsButton.isVisible = true
                                        iBinding!!.layoutHorizontal.visibility = View.GONE
                                        iBinding!!.newui.visibility = View.VISIBLE
                                    }
//                                            else if (i >= 1) {
//                                                val url =
//                                                    previousTabUrl1?.let { getFirstFiveFromUrl(it) }
...
//                                         Toast.makeText(this,"bug1",Toast.LENGTH_LONG).show()
...
//    private fun adjustVisibilityBasedOnUrl(url: String) {
//        if (url.isNullOrEmpty() || url == "fulguris://home" || url == "Bookmarks") {
//            Log.d("Debug>>", "Setting newui VISIBLE")
//
//            Toast.makeText(this,"visible",Toast.LENGTH_LONG).show()
//            // Your existing code
//        } else {
//            Toast.makeText(this,"notvisible",Toast.LENGTH_LONG).show()
//
//            Log.d("Debug>>", "Setting newui GONE")
//            // Your existing code
//        }
//    }
    ...
    override fun onResume() {
        ...
        if (currentTab != null) {
            if (currentTab.url.equals("")) {
                iBinding.uiLayout.visibility = View.VISIBLE
                iBinding.uiLayout.visibility = View.GONE
            } else {
                iBinding.uiLayout.visibility = View.GONE
                iBinding.uiLayout.visibility = View.VISIBLE
            }
    ...
    private fun searchTheWeb(query: String) {
        ...
        if (query.equals("Harav://homee") || query.equals("fulguris://homee")) {
            iBindingToolbarContent.addressBarInclude.search.setText("")
...
//            iBinding.newui.visibility= VISIBLE
...
//                val url1= currentTab?.url
//                val pos=currentTab.po
//                iBindingToolbarContent.addressBarInclude.search.setText(url1)
                    when {
                        url.isHomeUri() -> {
                            tabsManager.newTab(homePageInitializer, true)
...
//            currentTab?.addPageLoadListener(object : PageLoadListener {
//                override fun onPageFinished() {
//                    // Make the views visible when the query is properly loaded
//                    iBinding.newui.visibility = View.VISIBLE
//                    iBinding.layoutHorizontal.visibility = View.VISIBLE
//                }
//            })
    ...
    private fun applyToolbarColor(color: Int) {
        val tabs = iBinding.bottomNavigation.menu.findItem(R.id.nav_tabs)
        val tabs2 = iBinding.bottomNavigation.menu.findItem(R.id.nav_tabs)
...
//        }
        ...
        iBinding.searchSslStatuss.setColorFilter(currentToolBarTextColor)
    ...
    override fun changeToolbarBackground(favicon: Bitmap?, color: Int, tabBackground: Drawable?) {
        ...
        val currentTab = tabsManager.currentTab
       // Toast.makeText(this, "$url 1", Toast.LENGTH_SHORT).show()
        if (tabsManager.currentTab!!.url.equals("Harav://homee")||tabsManager.currentTab!!.url.equals("fulguris://homee"))
        {
            iBinding.newui.visibility= VISIBLE
            iBinding.layoutHorizontal.visibility= GONE
            iBinding.toolbarInclude.progressView.visibility=GONE
        }

        else if (currentTab?.canGoBack() == true) {
            iBinding.newui.visibility= GONE
            iBinding.layoutHorizontal.visibility= VISIBLE
            ...
            if (userPreferences.taskIcon) {
                ...
                setTaskDescription(
                    ActivityManager.TaskDescription(
                        getHeaderInfoText(userPreferences.taskLabel),
                        tab.favicon,
                        color
                    )
                )
...
//        iBinding.tabsButton.updateCount(number)
...
//       bookmarksView?.iBinding?.listBookmarks?.findViewHolderForAdapterPosition(0)?.itemView?.requestFocus()
    ...
    private fun doHideToolBar() {
        iBinding.toolbarInclude.toolbarLayout.visibility = View.GONE
    }
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            ...
            R.id.tabs_button -> if (configPrefs.tabBarInDrawer) openTabs() else {
                showMenuWebPage()
            }
...
//
//                val latestVersion =
//                    response.getJSONArray("versions").getJSONObject(0).getString("version_string")
//                if (latestVersion != BuildConfig.VERSION_NAME) {
//                    // We have an update available, tell our user about it
//                    makeSnackbar(
//                        getString(R.string.update_available) + " - v" + latestVersion,
//                        5000,
//                        if (configPrefs.toolbarsBottom) Gravity.TOP else Gravity.BOTTOM
...
//            searchView1.simulateTap()
//            searchView1.setText(aText)
//            searchView1.selectAll()
//            // Emulate tap to open up soft keyboard if needed
    ...
    fun showPageToolsDialog(position: Int) {
        ...
        BrowserDialog.showWithIcons(
            this, this.getString(R.string.dialog_tools_title),
            /*
            DialogItem(
    ...
    override fun onPreferenceStartFragment(
        ...
        val fragment =
            fragmentManager.fragmentFactory.instantiate(classLoader, preference.fragment!!)
...
//    {
//        if (Query.equals("fulguris://home"))
//        {
//
//            iBinding.newui.isVisible=true
//            iBinding.layoutHorizontal.isVisible=false
//
//            Log.d("perform>>>>>>", "p1")
//            Toast.makeText(this,"p1==",Toast.LENGTH_LONG).show()
//
...
//        {
//            Toast.makeText(this,"p1==",Toast.LENGTH_LONG).show()
//            iBinding.newui.isVisible=false
//            iBinding.layoutHorizontal.isVisible=true
//
//            Log.d("perform>>>>>>", "p2")
//        }
//
//    }
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        val currentTab = tabsManager.currentTab ?: return false // Return true if currentTab is null
        return when (item.itemId) {
            R.id.nav_home -> {
                currentTab.apply { requestFocus(); loadHomePage() }
                false // Indicate that the menu item click was handled successfully
            }
            R.id.nav_tabs -> {
                if (configPrefs.tabBarInDrawer) {
                    openTabs()
            ...
            else -> false // Return false for unknown menu item IDs
        }
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        val backwardMenuItem = menu.findItem(R.id.backward)
        val forwardMenuItem = menu.findItem(R.id.forward)

        tabsManager.currentTab?.apply {
            backwardMenuItem.isEnabled = canGoBack()
